...
1. 首先我们打开自己的HyperApp，然后从`商店`中找到MinecraftServer.
    <!--商店列表图片-->
    ![shop_img](/images/RD_MinecraftServerBuilding/HYPERAPP_SHOP.PNG)
2. 选中自己的服务器，并创建应用
    <!--应用将要指定服务器的图片-->
...
```
    ...
    Mods    —— 模型目录（如果服务器使用Mod，则需要指定Mod路径，默认为Data/mods）
...
11. 如果想使用盗版MC链接服务器，那么服务器需要关闭正版验证，在键盘上按下`i`键进入`INSERT`模式，注意低端文字的变化
    <!--INSERT截图-->
12. 我们需要修改的地方是这里，将`online-mode=`的`TRUE`更改为`false`来关闭正版验证
    <!--online-mode=TRUE-->
    <!--online-mode=false-->
